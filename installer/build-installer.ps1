<#
.SYNOPSIS
    Publishes SimCallouts and builds a versioned MSI installer.

.DESCRIPTION
    Reads <Version> from SimCallouts.csproj (the single source of truth for the app's
    version), republishes the self-contained app, and builds installer\SimCallouts-<version>.msi.
    ...
    manual uninstall needed. Just bump <Version> in SimCallouts.csproj before running this.
...
$csprojPath = Join-Path $root "src\SimCallouts\SimCallouts.csproj"
...
if (-not $version) {
    throw "Could not find <Version> in SimCallouts.csproj"
}
Write-Host "Building installer for SimCallouts v$version" -ForegroundColor Cyan

# --- Make sure SimCallouts isn't running (locks publish output files) ---
$running = Get-Process -Name "SimCallouts" -ErrorAction SilentlyContinue
if ($running) {
    throw "SimCallouts.exe is currently running. Close it before building, so the publish step can overwrite its files."
}
...
# --- Build the MSI ---
$outputMsi = Join-Path $installerDir "SimCallouts-$version.msi"
