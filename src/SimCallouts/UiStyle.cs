...
namespace SimCallouts
{
    ...
    public static class UiStyle
    {
        ...
        public static void StyleComboBox(ComboBox comboBox)
        {
            ...
            // FlatStyle.Flat still lets Windows draw the dropdown button with the native
            // (light) visual style underneath, which is what shows through as a bright box
            // regardless of BackColor - same native-theme override needed by the custom
            // rounded controls below, just applied to a stock ComboBox instead.
            if (comboBox.IsHandleCreated)
                NativeTheme.DisableVisualStyle(comboBox);
            else
                comboBox.HandleCreated += (_, _) => NativeTheme.DisableVisualStyle(comboBox);
        }
